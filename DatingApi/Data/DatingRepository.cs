...
namespace DatingApi.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientedId)
        {
            var messages = await _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
               .Include(u => u.Recipient).ThenInclude(p => p.Photos).
               Where(m => m.RecipientId == userId && m.RecipientDeleted == false && m.SenderId == recipientedId
               || m.RecipientId == recipientedId && m.SenderId == userId && m.SenderDeleted == false)
